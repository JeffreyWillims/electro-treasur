...
def upgrade() -> None:
    ...
    op.create_table('budget_plans',
        ...
        sa.Column('planned_amount', sa.Numeric(precision=12, scale=2), server_default=sa.text('0'), nullable=False),
        sa.ForeignKeyConstraint(['category_id'], ['categories.id'], ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('user_id', 'category_id', 'month', name='uq_budget_user_cat_month')
    )
    op.create_index(op.f('ix_budget_plans_category_id'), 'budget_plans', ['category_id'], unique=False)
