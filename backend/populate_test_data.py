...
from src.domain.models import Base, User, Category, CategoryType, Transaction, Budget
...
async def main():
    ...
    async with AsyncSessionLocal() as session:
        ...
        for name, ctype in categories_data:
            cat = (await session.execute(select(Category).where(Category.name == name, Category.user_id == 1))).scalar_one_or_none()
            if not cat:
        ...
        for i in range(90):
            ...
            # daily food ~1500
            t1 = Transaction(
                user_id=1, category_id=cats["Продукты"].id, 
            ...
            # transport ~500
            t2 = Transaction(
                user_id=1, category_id=cats["Транспорт"].id, 
            ...
            if i % 3 == 0:
                t3 = Transaction(
                    user_id=1, category_id=cats["Рестораны"].id, 
            ...
            if i % 30 == 0:
                # 150_000
                t_inc = Transaction(
                    user_id=1, category_id=cats["Зарплата"].id, 
