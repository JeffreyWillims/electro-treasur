...
async def test_transaction_idempotency(mock_db_session, mock_redis):
    ...
    assert len(errors) == 1, "The concurrent transaction must fail due to idempotency lock."
    assert "Idempotency key" in errors[0][1] or "locked" in errors[0][1].lower(), "Must fail with specific lock error."
...
async def test_dashboard_aggregation_algorithm(mock_db_session):
    ...
    cat1 = MagicMock(id=1, name="Mortgage"); cat1.id = 1; cat1.name = "Mortgage"
    cat2 = MagicMock(id=2, name="Food"); cat2.id = 2; cat2.name = "Food"
    ...
    mock_db_session.execute.side_effect = [
        ...
        MagicMock(scalars=lambda: MagicMock(all=lambda: [
            Transaction(category_id=1, amount=Decimal("1000.00"), executed_at=datetime(2025, 7, 15, 10, 0)),
            Transaction(category_id=2, amount=Decimal("200.00"), executed_at=datetime(2025, 7, 2, 10, 0)),
            Transaction(category_id=2, amount=Decimal("100.00"), executed_at=datetime(2025, 7, 2, 15, 0)),
        ]))
    ]
