...
эмодзи. Секции по приоритету, каждая включается только при наличии данных:
  ...
  3. Бюджеты      — худшая категория (≥80% лимита: «на исходе»/«пробит») или «в норме».
...
class RuleBasedInsightEngine:
    ...
    def _deficit_line(self, income: Decimal, expense: Decimal) -> str | None:
        ...
        if saved >= 0:
            return None
        return f"Расходы превысили доходы на {_format_money(-saved)} — период уходит в минус."
    ...
    def _budget_status(
        ...
        if worst is not None:
            ...
            state = "пробит" if ratio >= 1 else "на исходе"
            return (
                f"Бюджет «{name}» {state}: потрачено {_format_money(spent)} — "
                f"{ratio * 100:.0f}% лимита."
            )
        if any(b.amount_limit > 0 for b in budgets):
            return "Все бюджеты в пределах лимитов — хорошая дисциплина."
        return None
    ...
    def _anomaly(self, transactions: Sequence[TransactionData]) -> str | None:
        ...
        if count <= ANOMALY_DAILY_TX_LIMIT:
            return None
        return (
            f"Самый насыщенный день: {count} трат за сутки. Загляните в операции — "
            "не затесались ли импульсивные покупки."
        )
    ...
    def _recommendation(
        ...
        if saved < 0:
            ...
            return (
                f"Совет: чтобы выйти в ноль, сократите расходы минимум "
                f"на {_format_money(-saved)}{where}."
            )
