...
async def get_analytics_profile(session: AsyncSession, user_id: int) -> AnalyticsProfileResponse:
    ...
    cat_stmt = select(Category).where(Category.user_id == user_id, Category.type == CategoryType.expense)
    ...
    for cat in categories:
        ...
        cat_avgs.append(CategoryAvg(category_id=cat.id, name=cat.name, avg_amount=Decimal(round(avg_amt, 2))))
        
    # Get average income
    inc_stmt = select(func.sum(Transaction.amount)).join(Category).where(
        Transaction.user_id == user_id,
        Transaction.executed_at >= three_months_ago,
...
async def simulate_savings(request: SimulateRequest) -> SimulateResponse:
    ...
    optimized_monthly_addition = max(request.avg_income - optimized_expense_total + request.habit_savings, Decimal("0"))
